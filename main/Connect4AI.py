...
def main():
    ...
    while True:
        ...
        lcd.lcd_display_string('The %s player will go first.' % (turn), 1)
        mainBoard = getNewBoard()

        while True:
            if turn == 'human':
                lcd.lcd_display_string("Your Turn", 1)
                print('Your Turn')

#############################################
                move = camm.get_new_move(cam,picamera,Image,cv2,np)
                #move = getHumanMove(mainBoard) #Change to move = cameras output of human player move
                print('Your Turn %s' %(move))
################################################
                ...
                lcd.lcd_display_string("Computer Turn", 1)
                lcd.lcd_display_string("The computer is thinking...", 2)

                move = getComputerMove(mainBoard, computer)

#################################################
                arm.go_to_column(move)
                arm.open_grabber()
                arm.go_to_base()
                arm.off(GPIO)
#################################################
